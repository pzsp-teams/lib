...
func NewClientFromGraphClient(graphClient *graph.GraphServiceClient, senderConfig *SenderConfig, cacheEnabled bool, cachePath *string) (*Client, error) {
	...
	memberResolver := resolver.NewMemberResolverCacheable(channelsAPI, chatAPI, cache, cacheEnabled)

	teamSvc := teams.NewService(teamsAPI, teamResolver)
	channelSvc := channels.NewService(channelsAPI, teamResolver, channelResolver, memberResolver)
	chatSvc := chats.NewService(chatAPI, chatResolver, memberResolver)
